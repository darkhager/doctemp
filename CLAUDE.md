...
These commands are always available when working in this project:

| Command | When to use |
|---|---|
| `/arch-rules` | Before touching any backend file — review the 3-tier rules and ownership matrix |
| `/add-agent-method` | When adding a new method to any agent class |
| `/pre-commit-check` | Before every commit — architecture, tests, docs checklist |
| `/run-dev` | Start the dev environment or run tests |
